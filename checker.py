...
def printtrace(network, depth):
    ...
    # true = print to csv and false = print to terminal
    Printtocsv = False
    laststate = depth.pop()
    
    
    if (Printtocsv):    
        columns = {'gc', 'cost', 'soc', 'UHFtaken', 'L1taken', 'L2taken', 'Xtaken', 'sunActive'}
        ...
        for j in range(len(transitions)):
            if network.jump_np(secondlaststate, transitions[j]) == laststate:
                if not(Printtocsv):
                    ...
                    print("         ^^^")
                if Printtocsv:
                    temprow = pd.DataFrame({'gc' : [laststate.get_variable_value(0)], 'cost' : [0], 'soc' : [laststate.get_variable_value(1)], 'UHFtaken' : [laststate.get_variable_value(2)], 'L1taken' : [laststate.get_variable_value(4)], 'L2taken' : [laststate.get_variable_value(5)], 'Xtaken' : [laststate.get_variable_value(3)], 'sunActive' : [laststate.get_variable_value(6)]})
    ...
    if Printtocsv:
        df = df.iloc[::-1]
        df = df[['gc', 'cost', 'soc', 'UHFtaken', 'L1taken', 'L2taken', 'Xtaken', 'sunActive']]
        print(df)
...
def checkproperty(network, propertynumber):
    ...
    while True:
        ...
        Printtime = True
        if (count_it%10000 == 0 and Printtime):
            tmp_time = timer()
...
def checkxmin(network, propertynumber):
    ...
    while True:
        ...
        if(transnumber>=len(transitions)):
            ...
            print(current_state)
        Printtimeortrace = True
        if (count_it%5000 == 0 and Printtimeortrace):
            tmp_time = timer()
            hours, rem = divmod(tmp_time-start_time, 3600)
            minutes, seconds = divmod(rem, 60)
            print("     --- Time is = {:0>2}:{:0>2}:{:05.2f} ---".format(int(hours),int(minutes),seconds))
            ...
            print("number of iterations:", count_it)
            print("         length of state_cost:", len(state_cost))
            print("         length of checked_states:", len(checked_states))
            print("current state:\n", current_state)
        elif (Printtimeortrace == False):
            print("cost is: ",cost)
            print("current state:\n", current_state, "\n")
